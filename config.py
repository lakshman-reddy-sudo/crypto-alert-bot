...
Render note:
  ...
  web service. Our healthcheck server MUST listen on that port.
  Priority order: PORT (Render injects this) → HEALTHCHECK_PORT → 8080
"""
...
def _require(key: str) -> str:
    ...
    if not value:
        print(
            ...
            f"  Set it in Render → your service → Environment tab.\n",
            file=sys.stderr,
        )
        sys.exit(1)
    return value
...
def _healthcheck_port() -> int:
    ...
    Priority: PORT → HEALTHCHECK_PORT → 8080
    """
...
def _load() -> Settings:
    ...
    return Settings(
        ...
        # Healthcheck — reads Render's PORT automatically
