"""
cogs/alert_commands.py — All /alert slash commands as a discord.py Cog.

Extracted from bot.py so that:
  - Commands can be hot-reloaded without restarting the bot
  - bot.py stays a thin orchestration layer
  - Each command group can eventually live in its own Cog

Shared state (AlertCache, PriceStream) is injected via the constructor
...
class AlertCommands(commands.Cog):
    """Cog that owns the /alert command group and all sub-commands."""

    def __init__(self, bot: commands.Bot, cache: AlertCache, price_stream: "PriceStream") -> None:
        self.bot = bot
        self.cache = cache
